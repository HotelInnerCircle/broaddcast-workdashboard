...
export function ProfileForm({ phone }: { phone: string | null }) {
  ...
  const pw = useForm<PasswordInput>({ resolver: zodResolver(changePasswordSchema) });
  ...
  const savePassword = async (v: PasswordInput) => {
    try {
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <form onSubmit={pw.handleSubmit(savePassword)} noValidate>
        <Card>
          ...
          <CardContent className="space-y-4">
            <Field label="Current password" htmlFor="pw-current" error={pw.formState.errors.currentPassword?.message}><Input id="pw-current" type="password" autoComplete="current-password" {...pw.register("currentPassword")} /></Field>
            <Field label="New password" htmlFor="pw-new" error={pw.formState.errors.newPassword?.message} hint="At least 8 characters with a letter and a number."><Input id="pw-new" type="password" autoComplete="new-password" {...pw.register("newPassword")} /></Field>
          </CardContent>
