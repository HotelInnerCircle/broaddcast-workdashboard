...
function Column({ status, tasks, onMove }: { status: string; tasks: TaskRow[]; onMove: (task: TaskRow, status: string) => Promise<void> }) {
  ...
  return (
    <div ref={setNodeRef} className={cn("flex w-72 shrink-0 flex-col rounded-xl border border-border bg-muted/40 transition-colors", isOver && "border-primary bg-primary-soft/40")}>
