...
export function Avatar({ name, src, size = "md", className, fit = "cover" }: { name: string; src?: string | null; size?: keyof typeof sizes; className?: string; /** "contain" for logos so nothing is cropped. */ fit?: "cover" | "contain" }) {
  return (
    <AvatarPrimitive.Root className={cn("relative flex shrink-0 overflow-hidden rounded-[32%] bg-primary-soft", sizes[size], className)}>
      {src && <AvatarPrimitive.Image src={src} alt={name} className={cn("aspect-square h-full w-full", fit === "contain" ? "object-contain bg-white p-0.5" : "object-cover")} />}
